...
local Config = {
    ...
    -- (mobile: no keybinds, all actions via UI buttons)
...
local Connections = {}
local Unload          -- forward declaration; assigned below (kept local, no global leak)
local PlayerCharConnections = {}  -- forward-declared here so Unload and PlayerAdded share one table
...
function Chams.Create(target, colorData, label)
    ...
    -- destroy any leftover label from a previous session
    ...
    -- label is only created for players (passed explicitly); objects pass no label
    if label then
        ...
        if rootPart then
            ...
            bb.Name = "_ViolenceLabel"; bb.Size = UDim2.new(0,100,0,28)
...
function Chams.Update(target, newLabel, newDist)
    local d = Chams.Objects[target]; if not d or not d.textLabel then return end
    -- show name on first line, distance on second line (if distance is enabled)
    local text = newLabel or ""
...
-- Removes stale ESP/Chams entries whose game objects no longer exist
function ESP.cleanup()
    local valid = {}
    for _,p in ipairs(Players:GetPlayers()) do valid[p]=true end
    -- Player ESP: hide/destroy for players who left
    ...
    -- Object ESP: destroy Drawing objects for nil or stale entries
    -- (previously only nil entries were cleared; destroyed-part entries leaked)
    for key,esp in pairs(ESP.objectCache) do
        if not esp then
            ESP.objectCache[key]=nil
        else
            -- Check if any box line is still parented (proxy for "alive")
            -- Drawing objects from a finished match must be freed here
            local stale = false
            pcall(function()
                -- If the Drawing was already removed its Remove() will error; treat as stale
                if esp.Box and esp.Box[1] then
                    local ok = pcall(function() local _ = esp.Box[1].Visible end)
                    if not ok then stale = true end
                end
            end)
            if stale then
    ...
    -- Chams: remove for destroyed targets
    for target,_ in pairs(Chams.Objects) do
        if not target or not target.Parent then Chams.Remove(target) end
    end
end

-- Destroys ALL object ESP Drawing objects and clears objectCache entirely.
-- Called on match-end / lobby transition to guarantee no stale Drawings remain.
function ESP.clearObjectCache()
    for key, esp in pairs(ESP.objectCache) do
        ESP.hideObject(esp)
        ESP.destroyObject(esp)
        ESP.objectCache[key] = nil
...
function ESP.hideAll()
    ...
    -- Destroy (not just hide) object Drawing objects so they don't accumulate
    -- while ESP is disabled or when we're in lobby with no active match.
    ESP.clearObjectCache()
    Chams.ClearAll()
end
...
local function UpdateRadar()
    ...
    -- fix: remove excess dot Drawing objects instead of just hiding them to prevent accumulation
    for i=dotIdx+1,#Radar.dots do
        if Radar.dots[i] then pcall(function() Radar.dots[i]:Remove() end); Radar.dots[i]=nil end
    end
    local objIdx = 0
    ...
    -- fix: remove excess objectDot Drawing objects instead of just hiding them
    for i=objIdx+1,#Radar.objectDots do
        if Radar.objectDots[i] then pcall(function() Radar.objectDots[i]:Remove() end); Radar.objectDots[i]=nil end
    end
end

-- ─── Map scanning ────────────────────────────────
-- Each scan session gets a fresh counter so object ESP keys are unique
-- and never collide with keys from a previous match's (now-destroyed) objects.
local _espIdCounter = 0
local function NextEspId() _espIdCounter = _espIdCounter + 1; return _espIdCounter end

local function ScanMap()
    -- Fully destroy all object ESP Drawing objects and chams before rebuilding cache.
    -- This is the critical fix for post-match lobby lag: when the match ends abruptly
    -- (killer leaves), the old Generators/Hooks/etc parts are destroyed by the server.
    -- Without this, their Drawing objects keep rendering against nil/destroyed parts
    -- every frame, causing a severe FPS drop in lobby.
    ESP.clearObjectCache()
    Chams.ClearAll()
    ...
    -- Never fall back to Workspace:GetChildren() — in lobby there is no Map and scanning
    -- the entire workspace trashes the cache with lobby objects and causes severe lag.
    if not map then return end

    local objectsToScan = map:GetDescendants()
        ...
        if obj:IsA("Model") or obj:IsA("BasePart") then
            ...
            if obj:IsA("Model") then
                if n=="Generator" then 
                    ...
                    if p then table.insert(Cache.Generators,{model=obj,part=p,_espId=NextEspId()}) end 
                elseif n=="Hook" or n:find("Hook") then 
                    local p=obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")
                    if p then table.insert(Cache.Hooks,{model=obj,part=p,_espId=NextEspId()}) end 
                elseif n=="Pallet" then 
                    local p=obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")
                    if p then table.insert(Cache.Pallets,{model=obj,part=p,_espId=NextEspId()}) end 
                elseif n=="Window" or n:find("Window") then 
                    local p=obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")
                    if p then table.insert(Cache.Windows,{model=obj,part=p,_espId=NextEspId()}) end 
                end
            elseif obj:IsA("BasePart") then
                if n=="Gate" or n:find("ExitGate") or n:find("Gate") then 
                    table.insert(Cache.Gates,{part=obj,_espId=NextEspId()}) 
                end
            end
        end
    end
...
-- ─── Fly (Mobile D-Pad) ──────────────────────────
-- Based on the reference snippet. BodyVelocity/BodyGyro are parented to HRP
-- only while flying (startFlying/stopFlying pattern). Direction is driven
-- entirely by on-screen buttons — works on mobile with no keyboard needed.
-- The RenderStepped loop from the snippet is integrated into MainLoop so
-- there is only ONE RenderStepped connection for all per-frame logic.
...
local _flyActive = false  -- internal state, separate from Config.FLY_Enabled toggle
local _flyInputFlags = { forward=false, back=false, left=false, right=false, up=false, down=false }

local FlyGui = nil

local function _startFlying(root, hum)
    _flyActive = true
...
local function _stopFlying(hum)
    _flyActive = false
    FlyBodyVelocity.Parent = nil
    FlyBodyGyro.Parent     = nil
    if hum then hum.PlatformStand = false end
    -- Reset all input flags so buttons don't stay stuck
    for k in pairs(_flyInputFlags) do _flyInputFlags[k] = false end
end

local function _buildFlyGui()
    if FlyGui then return end

    local dpadSize = UDim2.new(0, 60, 0, 60)

    FlyGui = Instance.new("ScreenGui")
    FlyGui.Name         = "_ViolenceFlyButtons"
    FlyGui.ResetOnSpawn = false
    FlyGui.IgnoreGuiInset = true
    FlyGui.Parent       = LocalPlayer:WaitForChild("PlayerGui")

    -- UI-hide toggle button (top-right, always visible)
    ...
    flyControlsContainer.Parent             = FlyGui

    local function createBtn(parent, name, text, pos, size)
    ...
    local function tweenControls(visible)
    ...
    local function addTouch(btn, flagName)
        btn.MouseButton1Down:Connect(function() _flyInputFlags[flagName] = true  end)
        btn.MouseButton1Up:Connect(  function() _flyInputFlags[flagName] = false end)
        btn.MouseLeave:Connect(      function() _flyInputFlags[flagName] = false end)
    end

    -- D-pad: bottom-left area
    local fwd  = createBtn(flyControlsContainer, "FwdBtn",   "↑", UDim2.new(0,  70, 1, -190), dpadSize)
    local back = createBtn(flyControlsContainer, "BackBtn",  "↓", UDim2.new(0,  70, 1,  -70), dpadSize)
    local left = createBtn(flyControlsContainer, "LeftBtn",  "←", UDim2.new(0,  10, 1, -130), dpadSize)
    local rght = createBtn(flyControlsContainer, "RightBtn", "→", UDim2.new(0, 130, 1, -130), dpadSize)
    -- Up/Down: bottom-right
    local upB  = createBtn(flyControlsContainer, "UpBtn",   "Up",   UDim2.new(1, -110, 1, -190), dpadSize)
    local downB= createBtn(flyControlsContainer, "DownBtn", "Down", UDim2.new(1, -110, 1,  -70), dpadSize)

    addTouch(fwd,  "forward");  addTouch(back, "back")
    addTouch(left, "left");     addTouch(rght, "right")
    addTouch(upB,  "up");       addTouch(downB,"down")

    -- Hide/Show UI toggle
    local uiBtn = createBtn(FlyGui, "UIToggleBtn", "Hide UI", UDim2.new(0, 10, 0, 10), UDim2.new(0, 100, 0, 50))
    uiBtn.MouseButton1Click:Connect(function()
        uiVisible = not uiVisible
        tweenControls(uiVisible)
        uiBtn.Text = uiVisible and "Hide UI" or "Show UI"
    end)

    FlyGui.Enabled = false
end

-- Called from MainLoop (RenderStepped) — no extra RunService connection needed
local function UpdateFly()
    ...
    if Config.FLY_Enabled then
        ...
        if not _flyActive then _startFlying(root, hum) end

        -- Sync BodyVelocity/Gyro parent in case character respawned
        if FlyBodyVelocity.Parent ~= root then FlyBodyVelocity.Parent = root end
        if FlyBodyGyro.Parent     ~= root then FlyBodyGyro.Parent     = root end

        local cam = workspace.CurrentCamera
        local dir = Vector3.zero
        if _flyInputFlags.forward then dir = dir + cam.CFrame.LookVector  end
        if _flyInputFlags.back    then dir = dir - cam.CFrame.LookVector  end
        if _flyInputFlags.left    then dir = dir - cam.CFrame.RightVector end
        if _flyInputFlags.right   then dir = dir + cam.CFrame.RightVector end
        if _flyInputFlags.up      then dir = dir + Vector3.new(0, 1, 0)  end
        if _flyInputFlags.down    then dir = dir - Vector3.new(0, 1, 0)  end

        if dir.Magnitude > 0 then dir = dir.Unit end
        FlyBodyVelocity.Velocity = dir * Config.FLY_Speed
        FlyBodyGyro.CFrame       = cam.CFrame
    else
        if _flyActive then _stopFlying(hum) end
...
local function RemoveFog()
    -- Purge destroyed-instance keys that accumulated from previous matches.
    -- Without this, FogCache grows by ~10-30 entries per match indefinitely.
    for obj in pairs(FogCache) do
        if not obj or not obj.Parent then FogCache[obj] = nil end
    end
    pcall(function()
        local map = Workspace:FindFirstChild("Map")
        if map then
...
local function RestoreFog()
    pcall(function()
        for obj, data in pairs(FogCache) do
            -- guard against instances destroyed by the game between RemoveFog and RestoreFog
            if obj and obj.Parent ~= nil and data.parent then
                if obj:IsA("PostEffect") then
...
-- SkillCheck is called from ESPLoop at 20fps — no separate Heartbeat needed.
local _skillCheckRemote = nil
local function SetupSkillCheckMonitor()
    -- Pre-cache the remote once so we don't FindFirstChild every tick
    pcall(function()
        local r = ReplicatedStorage:FindFirstChild("Remotes"); if not r then return end
        local grem = r:FindFirstChild("Generator"); if not grem then return end
        _skillCheckRemote = grem:FindFirstChild("SkillCheckResultEvent")
    end)
end

local function UpdateSkillCheck()
    if not Config.AUTO_SkillCheck then return end
    if not _skillCheckRemote then
    ...
    pcall(function()
        for _, gen in ipairs(Cache.Generators) do
            if gen.part and (gen.part.Position - root.Position).Magnitude < 12 then
                local closestPoint = nil
                for _, child in ipairs(gen.model:GetChildren()) do
                    if child.Name:find("Point") then closestPoint = child; break end
                end
                _skillCheckRemote:FireServer("success", 1, gen.model or gen.part, closestPoint)
                break
            end
        end
    end)
...
local function RenderESP()
        ...
        if player~=LocalPlayer then
            ...
            if char and ((isK and Config.ESP_Killer) or (isS and Config.ESP_Survivor)) then
                if Config.ESP_PlayerChams then
                    ...
                    -- update name + live distance each render tick
                    local root = char:FindFirstChild("HumanoidRootPart")
                    local dist = root and GetDistance(root.Position) or nil
    ...
    -- objects
    -- IMPORTANT: keys use obj._espId (a stable integer assigned at ScanMap time),
    -- NOT tostring(instance). tostring() recycles the same strings across matches
    -- when new instances get the same name, so old Drawing objects were never
    -- freed — they just got orphaned. Integer IDs are unique per scan session.
    for _,obj in ipairs(Cache.Generators) do
        local key = obj._espId
        if Config.ESP_Generator and obj.part and obj.part.Parent then
            if Config.ESP_ObjectChams then
                if not Chams.Objects[obj.model or obj.part] then Chams.Create(obj.model or obj.part,ChamsColors.Generator) end
    ...
    for _,obj in ipairs(Cache.Hooks) do
        ...
        if Config.ESP_Hook and obj.part and obj.part.Parent then
            ...
            if Config.ESP_ObjectChams then
                if not Chams.Objects[target] then Chams.Create(target,uc) else Chams.SetColor(target,uc) end
                if ESP.objectCache[key] then ESP.hideObject(ESP.objectCache[key]) end
            else
                if not ESP.objectCache[key] then ESP.objectCache[key]=ESP.createObject(); ESP.setupObject(ESP.objectCache[key]) end
    ...
    for _,obj in ipairs(Cache.Pallets) do
        ...
        if Config.ESP_Pallet and obj.part and obj.part.Parent then
            if Config.ESP_ObjectChams then
                if not Chams.Objects[target] then Chams.Create(target,ChamsColors.Pallet) end
                if ESP.objectCache[key] then ESP.hideObject(ESP.objectCache[key]) end
            else
                if not ESP.objectCache[key] then ESP.objectCache[key]=ESP.createObject(); ESP.setupObject(ESP.objectCache[key]) end
    ...
    for _,obj in ipairs(Cache.Windows) do
        ...
        if Config.ESP_Window and obj.part and obj.part.Parent then
            if Config.ESP_ObjectChams then
                if not Chams.Objects[target] then Chams.Create(target,ChamsColors.Window) end
                if ESP.objectCache[key] then ESP.hideObject(ESP.objectCache[key]) end
            else
                if not ESP.objectCache[key] then ESP.objectCache[key]=ESP.createObject(); ESP.setupObject(ESP.objectCache[key]) end
    ...
    for _,obj in ipairs(Cache.Gates) do
        ...
        if Config.ESP_Gate and obj.part and obj.part.Parent then
            if Config.ESP_ObjectChams then
                if not Chams.Objects[obj.part] then Chams.Create(obj.part,ChamsColors.Gate) end
                if ESP.objectCache[key] then ESP.hideObject(ESP.objectCache[key]) end
            else
                if not ESP.objectCache[key] then ESP.objectCache[key]=ESP.createObject(); ESP.setupObject(ESP.objectCache[key]) end
...
local _speedHookConns  = {}
local _speedHookChar   = nil
local _speedWriting    = false  -- true while WE are writing, blocks our own signals
local _speedDeferred   = false  -- true if a task.defer(_forceApplySpeed) is already queued

local function _clearSpeedHooks()
    for _, c in ipairs(_speedHookConns) do
        pcall(function() c:Disconnect() end)
    end
    _speedHookConns  = {}
    _speedHookChar   = nil
    _speedWriting    = false
    _speedDeferred   = false
end

local function _forceApplySpeed(hum, char)
    _speedDeferred = false          -- consume the queued slot
    if _speedWriting then return end
    if not Config.SPEED_Enabled or Config.SPEED_Method ~= "Attribute" then return end
    if not hum or not hum.Parent then return end
    _speedWriting = true
    pcall(function() hum.WalkSpeed = Config.SPEED_Value end)
    pcall(function() hum:SetAttribute("WalkSpeed", Config.SPEED_Value) end)
    pcall(function() hum:SetAttribute("Speed",     Config.SPEED_Value) end)
    pcall(function() char:SetAttribute("WalkSpeed", Config.SPEED_Value) end)
    _speedWriting = false
end

local function _scheduleApplySpeed(hum, char)
    -- Only queue one deferred call at a time. If one is already pending,
    -- skip — it will run next frame and pick up the latest Config.SPEED_Value.
    if _speedDeferred then return end
    _speedDeferred = true
    task.defer(_forceApplySpeed, hum, char)
end

local function _installSpeedHooks(char, hum)
    _clearSpeedHooks()
    _speedHookChar = char

    local function onSpeedChanged()
        if _speedWriting then return end   -- our own write, ignore
        if Config.SPEED_Enabled and Config.SPEED_Method == "Attribute" then
            if hum.WalkSpeed ~= Config.SPEED_Value then
                _scheduleApplySpeed(hum, char)
            end
        end
    end

    local function onAttrChanged(attrName)
        if _speedWriting then return end
        if attrName == "WalkSpeed" or attrName == "Speed" then
            if Config.SPEED_Enabled and Config.SPEED_Method == "Attribute" then
                _scheduleApplySpeed(hum, char)
            end
        end
    ...
    local function onCharAttrChanged(attrName)
        if _speedWriting then return end
        if attrName == "WalkSpeed" then
            if Config.SPEED_Enabled and Config.SPEED_Method == "Attribute" then
                _scheduleApplySpeed(hum, char)
            end
        end
    ...
    table.insert(_speedHookConns, hum:GetPropertyChangedSignal("WalkSpeed"):Connect(onSpeedChanged))
    table.insert(_speedHookConns, hum.AttributeChanged:Connect(onAttrChanged))
    table.insert(_speedHookConns, char.AttributeChanged:Connect(onCharAttrChanged))
end

-- ─── KILLER_NoSlowdown implementation ────────────
-- Completely separate from the speed hack. Watches for WalkSpeed drops
-- (pallet stun / slowdown) and restores to baseline.
-- Aware of _speedWriting so it never conflicts with the speed hack's writes.
local _noSlowdownConn  = nil
local _noSlowdownChar  = nil
local _killerBaseSpeed = 16
local _noSlowdownApplying = false

local function _clearNoSlowdown()
    if _noSlowdownConn then
        pcall(function() _noSlowdownConn:Disconnect() end)
        _noSlowdownConn = nil
    end
    _noSlowdownChar      = nil
    _noSlowdownApplying  = false
end

local function _installNoSlowdown(char, hum)
    _clearNoSlowdown()
    _noSlowdownChar = char
    -- Snapshot the killer's normal speed. If SPEED_Enabled is also active,
    -- use that as the baseline so the two systems agree on the target.
    _killerBaseSpeed = (Config.SPEED_Enabled and Config.SPEED_Method == "Attribute")
        and Config.SPEED_Value
        or  hum.WalkSpeed

    _noSlowdownConn = hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
        if not Config.KILLER_NoSlowdown or GetRole() ~= "Killer" then return end
        -- If the speed hack is currently writing, this signal came from us — skip.
        if _speedWriting then return end
        if _noSlowdownApplying then return end

        local current = hum.WalkSpeed

        -- Keep baseline in sync with speed hack's target when both are active.
        if Config.SPEED_Enabled and Config.SPEED_Method == "Attribute" then
            _killerBaseSpeed = Config.SPEED_Value
        end

        if current < _killerBaseSpeed then
            -- A slowdown was applied — restore to baseline.
            _noSlowdownApplying = true
            task.defer(function()
                if hum and hum.Parent then
                    pcall(function() hum.WalkSpeed = _killerBaseSpeed end)
                end
                _noSlowdownApplying = false
            end)
        end
        -- Do NOT update _killerBaseSpeed here on the else branch.
        -- That caused the baseline to silently drift upward and then
        -- never trigger on the next slowdown.
    end)
end
...
local function UpdateSpeed()
    ...
    -- ── NoSlowdown hook management (independent of SPEED_Enabled) ──
    if Config.KILLER_NoSlowdown and GetRole() == "Killer" then
        if char ~= _noSlowdownChar then
            _installNoSlowdown(char, hum)
        end
    else
        if _noSlowdownChar ~= nil then _clearNoSlowdown() end
    end

    -- ── Speed hack ──────────────────────────────────────────────────
    -- If character changed, reinstall hooks (new spawn)
    if char ~= _speedHookChar then
        if Config.SPEED_Enabled and Config.SPEED_Method == "Attribute" then
            _installSpeedHooks(char, hum)
            _forceApplySpeed(hum, char)
        else
            _clearSpeedHooks()
        end
    end
    ...
    if Config.SPEED_Enabled then
        if Config.SPEED_Method == "Attribute" then
            -- Ensure hooks exist (e.g. method was just switched to Attribute)
            if _speedHookChar ~= char then
                _installSpeedHooks(char, hum)
                _forceApplySpeed(hum, char)
            end
            -- NOTE: do NOT call _forceApplySpeed here every loop tick.
            -- The reactive hooks handle resets. Calling it every 50ms was
            -- fighting the debounce stamp and causing missed resets.
    ...
    elseif _wasSpeedEnabled then
        ...
        pcall(function() hum:SetAttribute("WalkSpeed", 16) end)
        pcall(function() hum:SetAttribute("Speed", 16) end)
        pcall(function() char:SetAttribute("WalkSpeed", 16) end)
...
-- ─── Noclip ──────────────────────────────────────
-- ─── Noclip ──────────────────────────────────────
-- Persistent Heartbeat connection (matches the reference snippet logic).
-- Runs every frame while enabled, forcing CanCollide=false on all character
-- parts so the game can never re-enable collision between frames.
local NoclipConnection = nil

local function SetupNoclip()
    if NoclipConnection then NoclipConnection:Disconnect(); NoclipConnection = nil end
    -- Stepped fires once per physics step (~60fps but tied to physics not render).
    -- Only iterates LocalPlayer character, NOT all players.
...
-- Stub kept so ESPLoop call site compiles without changes
local function UpdateNoclip() end
...
local function BeatGameSurvivor()
    ...
    if State.LastFinishPos then
        if (exitPos - State.LastFinishPos).Magnitude > 50 then
            State.BeatSurvivorDone = false
        end
    end
...
local function BeatGameKiller()
    ...
    if needNewTarget then
        ...
        if not closest then State.KillerTarget = nil; return end
    ...
    local targetRoot = target.Character:FindFirstChild("HumanoidRootPart")
    local targetHum  = target.Character:FindFirstChildOfClass("Humanoid")
    if not targetRoot or not targetHum or not IsPlayerAlive(targetHum) then
        State.KillerTarget = nil; return
    end

    -- disable collision then teleport slightly behind target facing them
    pcall(function()
    ...
    if direction ~= direction then direction = Vector3.new(1, 0, 0) end  -- NaN guard
    root.CFrame = CFrame.new(targetPos + direction * 3 + Vector3.new(0, 1, 0), targetPos)
...
local function UpdateVisibility()
    -- Skip raycasting in lobby — no match, no ESP needed, and raycasts are expensive
    if GetRole() == "Lobby" then
        Cache.Visibility = {}
        return
    end
    for _,player in ipairs(Players:GetPlayers()) do
...
local function MainLoop()
    ...
    UpdateCameraFOV()
    UpdateAimbot()
    -- NOTE: RenderESP / UpdateRadar are throttled in ESPLoop, NOT here
end

-- Cache and Raycasting runs on its own throttled Heartbeat loop
local function ESPLoop()
    local _wasInLobby = (GetRole() == "Lobby")
    -- Track whether the Map existed last tick for instant destroy detection
    local _mapWasPresent = (Workspace:FindFirstChild("Map") ~= nil)

    -- Immediately free ALL ESP Drawing objects (players + objects), Chams, and cache tables.
    -- Called on any match->lobby or Map-destroyed transition.
    local function _flushMatchCache(now)
        -- Destroy player ESP Drawing objects. Players don't "leave" between matches
        -- so ESP.cleanup() never fires for them — these would otherwise accumulate
        -- across every match (23 Drawing objects per player per match).
        for player, esp in pairs(ESP.cache) do
            ESP.hide(esp); ESP.destroy(esp)
        ...
        ESP.clearObjectCache()
        Chams.ClearAll()
        Cache.Generators = {}; Cache.Gates = {}; Cache.Hooks = {}
        Cache.Pallets = {}; Cache.Windows = {}; Cache.Visibility = {}
        Cache.ClosestHook = nil
        -- Purge any stale destroyed-instance keys from FogCache
        for obj in pairs(FogCache) do
            if not obj or not obj.Parent then FogCache[obj] = nil end
        end
    ...
    while not State.Unloaded do
        local now = tick()
        local inLobby = (GetRole() == "Lobby")
        local mapPresent = (Workspace:FindFirstChild("Map") ~= nil)

        -- Map destroyed mid-session (killer leaves / abrupt match end).
        -- This fires BEFORE team assignment changes to "Lobby", so it is the
        -- fastest and most reliable signal for the abrupt-end scenario.
        if _mapWasPresent and not mapPresent then
            _flushMatchCache(now)
            _wasInLobby = true  -- treat as lobby immediately so next block skips
        end
        _mapWasPresent = mapPresent

        -- Lobby transition via team name (belt-and-suspenders)
        if inLobby and not _wasInLobby then
            _flushMatchCache(now)
        end
        _wasInLobby = inLobby

        -- Stale-cache guard: if cache still has objects but Map is gone, flush.
        if not mapPresent and (
            #Cache.Generators > 0 or #Cache.Hooks > 0 or
            #Cache.Gates > 0 or #Cache.Pallets > 0 or #Cache.Windows > 0
        ) then
            _flushMatchCache(now)
        end
        ...
        -- Cache/cleanup: every 8s, and only when in an active match (not lobby)
        if now - State.LastScanUpdate >= 8.0 then
            if GetRole() ~= "Lobby" then
                ScanMap()
            end
            State.LastScanUpdate = now
        ...
        -- ESP and Radar rendering (~20fps, throttled here instead of RenderStepped)
        ...
        -- Hitboxes: throttle to 200ms — no need to resize every 50ms
        if now - (State.LastHitboxUpdate or 0) >= 0.2 then
            UpdateHitboxes()
...
local function AutoLoop()
    while not State.Unloaded do
        task.wait(0.1)
        AutoAttack(); AutoHook(); AutoParry(); TeleportAway(); AutoWiggle()
        BeatGameSurvivor(); BeatGameKiller(); DoubleTap(); InfiniteLunge()
        DestroyAllPallets(); FullGenBreak()
    end
end
...
SecGen:Button({ Title="Leave Generator", Desc="TP away from nearest gen", Callback=function() LeaveGenerator(); Notif("Violence","Left generator",2) end })
SecGen:Button({ Title="Stop Auto Gen", Desc="Cancel repair immediately", Callback=function() Config.AUTO_Generator=false; Notif("Violence","Auto Gen stopped",2) end })
...
SecSpeed:Button({ Title="Toggle Speed Hack", Desc="Tap to enable / disable", Callback=function() Config.SPEED_Enabled=not Config.SPEED_Enabled; Notif("Speed",Config.SPEED_Enabled and "Speed ON" or "Speed OFF",2) end })
...
SecFly:Slider({ Title="Fly Speed", Value={Min=10,Max=200,Default=Config.FLY_Speed}, Step=5, Callback=function(v) Config.FLY_Speed=v; if FlyBodyVelocity then FlyBodyVelocity.Velocity = Vector3.zero end end })
SecFly:Button({ Title="Toggle Fly", Desc="Tap to enable / disable", Callback=function() Config.FLY_Enabled=not Config.FLY_Enabled; Notif("Fly",Config.FLY_Enabled and "Fly ON — use D-pad buttons" or "Fly OFF",2) end })
...
SecCollision:Button({ Title="Toggle Noclip", Desc="Tap to enable / disable", Callback=function() Config.NOCLIP_Enabled=not Config.NOCLIP_Enabled; Notif("Noclip",Config.NOCLIP_Enabled and "Noclip ON" or "Noclip OFF",2) end })
...
-- ─── Input (mobile: RMB aimbot only, no keyboard shortcuts) ─
...
-- ─── Unload ──────────────────────────────────────
-- fix: assigned to the local declared above (was a bare global, polluting _G)
Unload = function()
    State.Unloaded=true
    ...
    pcall(function()
        for player,orig in pairs(OriginalHitboxSizes) do
            -- fix: guard against player having left between hitbox-set and unload
    ...
    for player, conn in pairs(PlayerCharConnections) do
...
-- ─── LocalPlayer CharacterAdded: reapply speed hooks on every spawn ──────────
LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(0.5) -- wait for Humanoid to fully initialise / server to push first props
    ...
    -- Only rescan if we are in an active match. Lobby respawn has no Map to scan.
    if GetRole() ~= "Lobby" then ScanMap() end
end)

-- PlayerCharConnections is forward-declared at the top of the script.
Connections.PlayerLeft=Players.PlayerRemoving:Connect(function(player)
    ...
    ESP.velocityData[player]=nil  -- fix: was never cleared, causing stale velocity entries
    -- Disconnect and remove stored CharacterAdded connection
    if PlayerCharConnections[player] then
        pcall(function() PlayerCharConnections[player]:Disconnect() end)
        PlayerCharConnections[player] = nil
    end
end)

Connections.PlayerAdded=Players.PlayerAdded:Connect(function(player)
    -- Disconnect any previous connection for this player before creating a new one
    if PlayerCharConnections[player] then
        pcall(function() PlayerCharConnections[player]:Disconnect() end)
    end
    PlayerCharConnections[player] = player.CharacterAdded:Connect(function()
        task.wait(1)
        -- Only rescan if we are in an active match. When a match ends, all players
        -- respawn to lobby simultaneously which would cause a burst of ScanMap calls
        -- against Workspace (no Map present), thrashing the cache and causing lag.
        if not State.Unloaded and GetRole() ~= "Lobby" then ScanMap() end
    end)
end)
