...
-- ─── Auto Skill Check (UI-based timing) ─────────
-- Watches SkillCheckPromptGui visibility, then on each Heartbeat checks
-- whether the rotating Line is inside the Goal success window and fires
-- the mobile action button at the correct moment instead of spamming remotes.
local SC_TouchID        = 8822
local SC_ActionPath     = "Survivor-mob.Controls.action.check"
local SC_HeartbeatConn  = nil
local SC_VisibilityConn = nil

local GuiService = game:GetService("GuiService")
local VirtualInputManager = game:GetService("VirtualInputManager")

local function SC_GetActionButton()
    local current = LocalPlayer:WaitForChild("PlayerGui")
    for segment in string.gmatch(SC_ActionPath, "[^%.]+") do
        current = current and current:FindFirstChild(segment)
    end
    return current
end

local function SC_TriggerButton()
    local b = SC_GetActionButton()
    if b and b:IsA("GuiObject") then
        local p, s, inset = b.AbsolutePosition, b.AbsoluteSize, GuiService:GetGuiInset()
...
local function SetupSkillCheckMonitor()
    task.spawn(function()
        ...
        if SC_VisibilityConn then SC_VisibilityConn:Disconnect() end
        SC_VisibilityConn = check:GetPropertyChangedSignal("Visible"):Connect(function()
            if not Config.AUTO_SkillCheck then
            ...
            if LocalPlayer.Team and LocalPlayer.Team.Name == "Survivors" and check.Visible then
                -- Skill check appeared — start tracking the needle
                if SC_HeartbeatConn then SC_HeartbeatConn:Disconnect() end
                SC_HeartbeatConn = RunService.Heartbeat:Connect(function()
                    if not Config.AUTO_SkillCheck then
                        SC_HeartbeatConn:Disconnect(); SC_HeartbeatConn = nil
                        return
                    end
                    local lr = line.Rotation % 360
                    local gr = goal.Rotation % 360
                    -- Success zone: +101° to +115° ahead of the goal marker
                    local ss = (gr + 101) % 360
                    local se = (gr + 115) % 360
                    local inZone = (ss > se)
                        and (lr >= ss or lr <= se)
                ...
                -- Skill check dismissed — stop tracking
                if SC_HeartbeatConn then SC_HeartbeatConn:Disconnect(); SC_HeartbeatConn = nil end
            end
        end)

        -- Store refs so Unload can clean them up
        QTEHandler.Conn   = SC_HeartbeatConn
        QTEHandler.UIConn = SC_VisibilityConn
    end)
end
...
SecFly:Button({ Title="Toggle Fly", Desc="Tap to enable / disable", Callback=function() Config.FLY_Enabled=not Config.FLY_Enabled; Notif("Fly",Config.FLY_Enabled and "Fly ON" or "Fly OFF",2) end })
...
Unload = function()
    ...
    -- Disconnect skill check monitor connections
    pcall(function()
        if SC_HeartbeatConn then SC_HeartbeatConn:Disconnect(); SC_HeartbeatConn = nil end
        if SC_VisibilityConn then SC_VisibilityConn:Disconnect(); SC_VisibilityConn = nil end
    end)
    -- Destroy WindUI
